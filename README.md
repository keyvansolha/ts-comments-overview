...
- **موبایل:** نوار چسبان موبایل تب نظرات ندارد (فقط مشخصات/معرفی/متداول)؛ ورودی
  نظرات در موبایل میان‌بر `data-action="comments"` کنار دکمه‌ی علاقه‌مندی است و
  حلقه روی همان می‌نشیند.
- نشان‌گذاری با `assets/js/comments-overview.js` انجام می‌شود، نه با دست‌کاری
  قالب: اسکریپت وجود بخش خلاصه در DOM را بررسی می‌کند و کلاس `ts-co-nav-glow` را
  می‌گذارد. اگر محصول خلاصه نداشته باشد (از جمله وقتی آستانه‌ی کیفیت جلوی آن را
  گرفته) هیچ تغییری در صفحه دیده نمی‌شود.
- حلقه با `outline` کشیده می‌شود و رنگ آن با انیمیشن CSS بین رنگ‌های رنگین‌کمان
  می‌چرخد. دو دلیل: قالب در `product-detail-ui.css` تمام شبه‌عنصرهای تب
  (`::before`/`::after`) را با `display:none !important` خاموش کرده است، و
  `outline` هیچ اثری روی چیدمان و هیچ درگیری‌ای با `background`/`border` خود قالب
  ندارد (هم `border-radius` تب ۸px و هم پیل موبایل ۵۰px را دنبال می‌کند).
- `prefers-reduced-motion: reduce` انیمیشن را خاموش و حلقه را به رنگ تأکید سایت
  ثابت می‌کند.
...
assets/js/comments-overview.js                      نشان‌گذاری ورودی «نظرات» (حلقه‌ی رنگین‌کمانی)
